...
''';
    ...
    if (isAny) {
      ...
      categoryRule =
          '요청된 카테고리는 "한식"입니다. 오직 한국 전통 음식만 추천하세요. **절대 금지: 중식(짜장면, 짬뽕, 마파두부, 팔보채), 일식(스시, 라멘, 우동), 양식(파스타, 피자, 스테이크)**';
    } else if (category == '중식') {
      categoryRule =
          '요청된 카테고리는 "중식"입니다. 오직 중국 음식만 추천하세요. **절대 금지: 한식(김치찌개, 된장찌개, 비빔밥), 일식(스시, 라멘), 양식(파스타, 피자)**';
    } else if (category == '일식') {
      categoryRule =
      ...
      categoryRule =
          '요청된 카테고리는 "분식"입니다. 오직 분식점 메뉴만 추천하세요. **절대 금지: 한식(김치찌개, 갈비탕), 중식(짜장면, 짬뽕, 팔보채, 마파두부), 일식(스시, 라멘)**';
    ...
    final examples = '''
예시(각 카테고리는 완전히 독립적임):
- 한식 ONLY: 김치찌개, 된장찌개, 비빔밥, 불고기, 제육볶음, 닭갈비, 갈비탕, 냉면, 삼계탕, 순두부찌개
- 중식 ONLY: 짜장면, 짬뽕, 마라탕, 마라샹궈, 마파두부, 깐풍기, 볶음밥, 딤섬, 훠궈, 우육면
- 일식 ONLY: 스시, 사시미, 라멘, 우동, 돈카츠, 규동, 오코노미야키, 텐동, 야키토리
- 양식 ONLY: 파스타, 피자, 스테이크, 리조또, 라자냐, 감바스 알 아히요
- 분식 ONLY: 떡볶이, 순대, 오뎅, 김밥, 라볶이, 쫄면
- 아시안 ONLY: 쌀국수, 팟타이, 똠얌꿍, 반미, 카오팟, 분짜, 나시고랭, 미고랭, 커리
- 패스트푸드 ONLY: 햄버거, 프라이드치킨, 핫도그, 타코
- 편의점 ONLY: 신라면, 짜파게티, 삼각김밥, 도시락, 샌드위치, 컵라면
- 카페 ONLY: 아메리카노, 카페라떼, 카페모카, 카푸치노, 에스프레소, 바닐라라떼
''';
...
- **개수: 정확히 50개**를 생성하세요.
- **각 음식명 앞에 반드시 번호를 붙이세요.** (예: "1. 짜장면", "2. 짬뽕", "3. 탕수육", ...)
- 출력은 오직 순수 JSON 배열만. 설명/문장은 금지. 마크다운 금지.
- JSON 형식: [{ "name":"1. 메뉴명"}, { "name":"2. 메뉴명"}, { "name":"3. 메뉴명"}, ...]

$examples
이제 정확히 50개의 음식을 번호와 함께 JSON 배열로만 출력하세요.
''';
  }
